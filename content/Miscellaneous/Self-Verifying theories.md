...
Gödel encoding is usually defined via addition, + and multiplication, $\times$ via prime factorization (i.e., $p_{1}^{n_{1}} \times p_{2}^{n_{2}} \times\dots$,etc. where  is a prime number which is assigned to the position of a symbol in the sentences , and $n_{i} \in \mathbb{Z}^+$ assigned to a particular logical symbol and $i \in \mathbb{N}$ is an index), see [[Gödel's Incompleteness Theorems]]. This is needed to express diagonalization:
