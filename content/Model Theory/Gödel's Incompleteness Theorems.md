...
Gödel numbering allows us to encode formulae of the object theory into objects of the object theory (in this case arithmetic numbers). 'Internalization' is the process of expressing a meta statement about the object language w/in itself, this is done by the object language encoding its own syntax. An example of this is seen below via considering the statement "$T$ proves the sentence $\varphi$", which can be encoded into the object language as $\exists y\text{Prf}(m, \lceil \varphi \rceil)$ where $m$ is the Gödel number of a proof in $T$ of the formula w/ Gödel number $n$. These processes allows us to express self-referential statement which is integral to Gödel's Incompleteness Theorems and Tarski's Undefinability Theorem. The internalization (the encoding of the object language) itself is carried out in the metatheory.
...
Gödel encoding (partially) internalizes the metatheory (w/in the object theory), allowing us refer to it and talk about proofs over its own statements. Specifically, only meta statements about the  object language are encoded within itself. This is limited access to the metatheory as via [Tarski's theory of truth](Logic/Tarski's%20theory%20of%20truth.md), the object language cannot refer to its own semantics nor the semantics and syntax of any theory of a higher meta-level (as this would cause semantic collapse). Thus, we cannot encode semantic notions, only syntactic ones like proofs.
